...
import { createDraftJournalFromForm } from '../actions';
...
export function JournalForm({ accounts }: Props) {
  const [rows, setRows] = useState<JournalRow[]>([newRow(), newRow()]);
  ...
  return (
    <form action={createDraftJournalFromForm} className="space-y-6 border rounded-md p-4 bg-card">
      ...
      <div className="space-y-1">
        ...
        <input
          ...
          type="text"
          required
          placeholder="e.g. Prepaid insurance adjustment"
      ...
      <div className="flex items-center justify-between">
        ...
        <SubmitButton disabled={isOutOfBalance} />
      </div>

      <p className="text-xs text-muted-foreground">
        Leave unused lines blank. Debits and credits must balance; unbalanced entries will be
        rejected.
      </p>
    </form>
  );
